...
- Keep code concise, maintainable, and easy to change later. Avoid hacky, workaround, brittle, or overcomplicated solutions.
- Do not write redundant code: redundancy includes but is not limited to duplicate logic in different places, tiny helpers that are only used once, small code files for constants or helpers that are only imported once, explicit types that can be inferred, and unnecessary wrappers that provide no real value unless they clearly improve readability, create real reuse, or define a useful boundary. Also constants that are only used once should be inlined, you may add a short comment to explain the value if it is not obvious.
...
- Use helpful comments when they make code easier to read or maintain; keep them concise and plain (do not use fancy hard to understand wording). You are encouraged to add comments to explain the reasoning behind non-obvious code, but do not add comments that just restate what the code does or add unnecessary noise.
